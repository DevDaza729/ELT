...
SELECT 
	...
	AVG(CASE WHEN year = '2016' THEN estimate_time ELSE NULL END) AS Year2016_estimated_time,
	AVG(CASE WHEN year = '2017' THEN estimate_time ELSE NULL END) AS Year2017_estimated_time,
	AVG(CASE WHEN year = '2018' THEN estimate_time ELSE NULL END) AS Year2018_estimated_time
FROM 
	(
	SELECT
    ...
    JULIANDAY(order_estimated_delivery_date)- JULIANDAY(order_purchase_timestamp) AS estimate_time
    FROM olist_orders oo 
	WHERE order_status = 'delivered' AND order_delivered_customer_date IS NOT NULL) AS P
GROUP BY month_no;