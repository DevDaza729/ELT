...
SELECT
    ...
    CASE
        ...
        ELSE 0
    END AS month,
    sum(CASE WHEN a.year = '2016' THEN payment_value END) AS Year2016,
    sum(CASE WHEN a.year = '2017' THEN payment_value END) AS Year2017,
    sum(CASE WHEN a.year = '2018' THEN payment_value END) AS Year2018
FROM
    (
    SELECT
        customer_id,
    ...
    GROUP BY 2
    ORDER BY order_delivered_customer_date ASC) AS a
GROUP BY
    month
ORDER BY
    month_no ASC;