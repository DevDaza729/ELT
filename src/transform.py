...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = orders_items.merge(products, on="product_id")
    ...
    product_weight_g=("product_weight_g", "sum")).reset_index()
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    # Rename to avoid confusion with the column name
    order_purchase_ammount_per_date.rename(columns={"order_purchase_timestamp": "order_count"}, inplace=True)
    
    # Reset the index to have the date as a column
    order_purchase_ammount_per_date.reset_index(names='date', inplace=True)

    # Convert date to datetime to merge with holidays
    order_purchase_ammount_per_date["date"] = pd.to_datetime(order_purchase_ammount_per_date["date"], format="%Y-%m-%d")
    ...
    result_df_order = order_purchase_ammount_per_date.merge(holidays, on="date", how="left")
    result_df_order["holiday"].fillna(False, inplace=True)
    result_df_order = result_df_order.reindex(columns=["order_count", "date", "holiday"])


    # Keep the code below as it is, this will return the result from
    # `aggregations` variable with the corresponding name and format.
    return QueryResult(query=query_name, result=result_df_order)
